...
async function getRecommendations(): Promise<Recommendation[]> {
  try {
    ...
    if (!res.ok) {
      console.error("Recommendations fetch failed:", res.status, res.statusText);
      return [];
    }
    ...
    if (!Array.isArray(data)) {
      console.error("Recommendations response is not an array:", data);
      return [];
    }
          ...
          {recommendations.length === 0 ? (
            <div className="bg-white rounded-[1.5rem] p-8 shadow-md border border-gray-100">
              ...
              <p className="text-gray-600">
                This usually means the backend is not running or the API did not
                return data. Make sure FastAPI is running on:
              </p>
              <p className="mt-2 font-mono text-sm text-[#007788]">
                http://127.0.0.1:8000
              </p>
            </div>
          ) : (
